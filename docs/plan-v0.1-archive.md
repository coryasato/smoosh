# Smoosh — v0.1 development archive

Full development history for v0.1 (M1-M12), moved out of `PLAN.md` on 2026-08-13 to keep the live
plan lean going into the next round (UI polishing). This is reference material — the exact
reasoning behind each decision, live-verification writeups, and mutation-testing findings — not a
living document. `PLAN.md` holds the current, evergreen state (technical approach, testing
strategy, what's still open); this file holds how it got there.

## Original design sketches (pre-implementation)

These predate `src/main.zig`/`src/app.native` and were superseded once the real code landed (M2
replaced the sketches below with the real `Model`/`Msg`, M9 replaced the UI sketch with the real
view). Kept here for the historical record of original intent, not as a current reference — read
the real source for the current shape.
...
The tree still carried the `native init` counter scaffold going into M2:
...
and the open dialog worked. No separate `dialog` permission was needed.
...
Ordered by dependency. The file-acquisition unknown was resolved before M1; nothing was left to spike.
Bracketed refs are the MVP success-criteria checkboxes each milestone satisfied.
...
Enforces **100 MB** / **50 megapixels** at file-load time (the top of the considered 80-100MB/40-50MP
...
`reset`.
Settled/found here:
- **The original plan — reading dimensions off the thumbnail spawn's own `-g` flags — does not
...
`sips` really drives) rather than retry the OS-level automation. **Flag for whoever runs M4's live check later**:
...
fixture and pin the confirmed argv into the "Encoding strategy" section.
...
- **Encoding a `.webp` source to WebP is skipped, not performed** (`same_path`): the destination would BE
  ...
  OUTPUT, never the user's original.
...
auto-save.
Settled/found here:
- **"Both" mode needed a real design decision the sketch didn't cover**: `showSaveDialog`
  ...
  idiom but one never previously planned) or disabling Save As for "Both" (rejected: Both is the headline
...
- **`save_as_result` changed type from the sketch** (`fx.writeFile`'s `EffectFileResult`) **to
...
- **The drop zone says "click", not "drop".** The original sketch reads "Drop image here"; file drops
  were an open question and did not work in v0.1 yet, so the zone (and `statusLine`'s idle text, changed to
...
- **Error messages stopped naming the file.** `<status-bar>` is one honest line — it takes no `wrap`
  ...
  else ("Not an image Smoosh can read. Try JPEG, PNG, HEIC or WebP."). The "Status → error
...
The open question ("whether a drop-zone `on-file-drop` markup hook exists") has an
...
`"dialog"`. An earlier draft of CLAUDE.md claimed no evidence either exists — that was wrong.
Both carry a `void` payload and nothing in the runtime reads them,
...
and `check` went quiet — running `test` before `check` applies after every upgrade.)
...
- **The two entry points share one helper, `beginLoad(model, fx, path)`**, factored out of what used
  ...
  claim that "a drop re-enters the already-validated chain with zero new pipeline" — after
...
- **`onDrop` is `pub`, matching `formatBytes`/`formatSavings`/`resolveSpawnEnviron`'s precedent** for a
  pure helper worth testing directly rather than only through the dispatch path — the testing
...
- **`app.zon` gained `"file_drops"` in `.capabilities`.** Confirmed real (`app_manifest.CapabilityKind
  ...
  earlier "no evidence either exists, do not add them speculatively" line was corrected — it was wrong about
...
- **The drop zone's copy changed from "Click to choose an image" to "Drop an image here — or click to
  ...
  because it didn't work yet ("a drop zone that lies about accepting drops is the
...
system-tool gap a picked HEIC would hit identically, unrelated to drops, and out of M11's scope (fixed
later in M12, below).
...
sitting untouched. This is "one full smoosh from a dropped file," done.
...
Chose the "fix it in Phase A" direction over narrowing the promise: `smoosh` now stages a HEIC/HEIF
source as a PNG via `sips` before either encoder runs, and both encoders read the staged PNG instead
of the original file. `avifenc`/`cwebp` never see a HEIC container at all.
Settled/found here:
...
This closes the HEIC known limitation M11 left open.

## Resolved decisions, in full

Short version lives in `PLAN.md`'s "Key decisions" section — this is the full rationale behind each.
